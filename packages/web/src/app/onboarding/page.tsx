...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    setUserName(auth.user.name ?? auth.user.email.split('@')[0] ?? '');
  }, [router]);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(ellipse at 50% 20%, rgba(245,158,11,0.08) 0%, transparent 65%)',
        }}
      />
      ...
      <div className="relative max-w-lg w-full">
          ...
          <div className="terminal-cmd">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs" style={{ color: 'var(--muted)' }}>
                Paste and run this command once
              </span>
            </div>
        ...
        >
          {copied ? '✓ Copied — now paste it in your terminal' : 'Copy command'}
        </button>
          ...
          {[
            {
              ...
              sub: 'Any terminal, any directory — it works globally',
            },
            {
              n: 2,
              title: 'Paste and run the command above',
              sub: 'Claude will ask you to sign in via browser — just click Allow',
            },
            {
              n: 3,
              title: 'Open Claude Code and ask away',
              sub: '"What tasks do I have today?" or "Create a high-priority bug report"',
            },
          ].map(({ n, title, sub }) => (
            <div
              key={n}
        ...
        <div className="animate-fade-up delay-400 flex flex-col sm:flex-row gap-3">
          <Link
            href="/dashboard"
            className="amber-btn flex-1 py-3 rounded-lg text-sm"
          >
            Go to dashboard →
          </Link>
          <a
